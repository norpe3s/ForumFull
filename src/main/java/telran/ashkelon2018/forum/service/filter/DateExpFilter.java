...
@Service
@Order(2)
public class DateExpFilter implements Filter {
	...
	public void doFilter(ServletRequest reqs, ServletResponse resp, FilterChain chain)
		...
		if (!path.startsWith("account/password") && token != null && (!( path.startsWith("/account")&&"POST".equals(method) ))) {
			...
			if(userAccount.getExpdate().isBefore(LocalDateTime.now())) {
				response.sendError(403, "Password expired." + (configuration.getLogicviewError()?" // Error 4X030008":"") );
			}
